...
from train_helper import Client
class Type(Enum):
    crossing = "crossing"
    high_way = "high_way"
    main_road = "main_road"
...
class Environment:
    ...
    def __init__(self, _type:str):
        ...
        self.latency = [0.4987, 0.5822, 0.4846, 0.4974, 0.3972, 0.2352, 0.4582, 0.1177, 0.3522, 0.2663,]
        self.client_num = 10
        self._type = _type
        self.lamda = 0.5

        self.client = [Client("./new_data_25/client{}.data".format(i), "client" + str(i))  for i in range(10)]
        self.tmp_models = [torch.load("./modelzoo/coco2017-0.241078ap-model.pth", map_location=self.device) for i in range(10)]
        self.now_model = torch.load("./modelzoo/coco2017-0.241078ap-model.pth", map_location=self.device)
        self.epoch = 100
