...
class AutoCoder1(nn.Module):
    def __init__(self, device) -> None:
        super().__init__()
        self.conv11 = nn.Conv1d(kernel_size=25, stride=1, in_channels=1, out_channels=32, padding=12).to(device)
        self.pool11 = nn.MaxPool1d(stride=16, kernel_size=16, return_indices=True).to(device)
        self.bn11 = nn.BatchNorm1d(32).to(device)
        self.conv12 = nn.Conv1d(kernel_size=25, stride=1, in_channels=32, out_channels=64, padding=12).to(device)
        self.pool12 = nn.MaxPool1d(stride=16, kernel_size=16, return_indices=True).to(device)
        self.bn12 = nn.BatchNorm1d(64).to(device)
        self.conv13 = nn.Conv1d(kernel_size=25, stride=1, in_channels=64, out_channels=128, padding=12).to(device)
        self.pool13 = nn.MaxPool1d(stride=16, kernel_size=16, return_indices=True).to(device)
        self.bn13 = nn.BatchNorm1d(128).to(device)
        self.conv14 = nn.Conv1d(kernel_size=25, stride=1, in_channels=128, out_channels=256, padding=12).to(device)
        self.pool14 = nn.MaxPool1d(stride=16, kernel_size=16, return_indices=True).to(device)
        self.bn14 = nn.BatchNorm1d(256).to(device)


        self.dnn11 = nn.Linear(1024, 256).to(device)
        self.dnn12 = nn.Linear(256, 16).to(device)

        self.dnn21 = nn.Linear(16, 256).to(device)
        self.dnn22 = nn.Linear(256, 1024).to(device)

        self.conv21 = nn.ConvTranspose1d(kernel_size=25, stride=1, in_channels=256, out_channels=128, padding=12).to(device)
        self.pool21 = nn.MaxUnpool1d(stride=16, kernel_size=16).to(device)
        self.bn21 = nn.BatchNorm1d(128).to(device)
        self.conv22 = nn.ConvTranspose1d(kernel_size=25, stride=1, in_channels=128, out_channels=64, padding=12).to(device)
        self.pool22 = nn.MaxUnpool1d(stride=16, kernel_size=16).to(device)
        self.bn22 = nn.BatchNorm1d(64).to(device)
        self.conv23 = nn.ConvTranspose1d(kernel_size=25, stride=1, in_channels=64, out_channels=32, padding=12).to(device)
        self.pool23 = nn.MaxUnpool1d(stride=16, kernel_size=16).to(device)
        self.bn23 = nn.BatchNorm1d(32).to(device)
        self.conv24 = nn.ConvTranspose1d(kernel_size=25, stride=1, in_channels=32, out_channels=1, padding=12).to(device)
...
class AutoCoderModelHelper:
    ...
    def get_squad(self, ):
        ...
        result = []
        for _t in _ts:
            item = get_params_by_url(_t).cpu().numpy()
            result.append(item)
        return result
...
if __name__ == '__main__':
    ...
    #     for j in range(int(len(a) / 10)):
    #         print(_t - time.time())
    #         _t = time.time()
    #         helper.optimizer.zero_grad()
    #         b = torch.stack(a[j * 10: (j + 1) * 10], dim=0).to(helper.device, torch.float)
