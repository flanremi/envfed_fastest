...
if __name__ == '__main__':
    ...
    for type_name in type_names:
        for pos in range(25):
            ...
            cfg = utils.utils.load_datafile(data_url.format(type_name, pos))

            val_result = []

            dir_url = cfg["model_name"]
            ...
            # 初始化模型结构
            model = detector.Detector(cfg["classes"], cfg["anchor_num"], load_param).to(device)
            summary(model, input_size=(3, cfg["height"], cfg["width"]))

            # 加载预训练模型参数
            if load_param == True:
                premodel = torch.load(premodel_path, map_location=device)
            ...
            for epoch in range(cfg["epochs"]):
                ...
                if True:
                    ...
                    val_result.append((precision, recall, AP, f1))
                    torch.save(model.state_dict(), cfg["model_name"] + "epoch_{}.pth".format(epoch))

                # 学习率调整
                scheduler.step()
            with open(cfg["model_name"] + "val_result", "w") as file:
                file.write(json.dumps(val_result))